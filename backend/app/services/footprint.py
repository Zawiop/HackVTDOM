...
Lives in services rather than the router so the seeder and step 08 can resolve a
footprint without going through HTTP.
"""
...
from ..models.contracts import FootprintCandidate, FootprintResult
from . import cache, geo_math, overpass
...
class FootprintUnavailable(RuntimeError):
    """Every Overpass endpoint failed and nothing was cached."""


async def lookup(lat: float, lng: float, radius_meters: Optional[int] = None) -> FootprintResult:
    settings = get_settings()
    match_radius = radius_meters or settings.overpass_match_radius_meters
    ...
    cached = cache.get(lat, lng, neighbor_radius)
    if cached is not None:
        elements, source = cached
        was_cached = True
    else:
        try:
            elements, source = await overpass.fetch_buildings(lat, lng, neighbor_radius)
        except overpass.OverpassError as exc:
            logger.warning("Overpass failed at %s,%s: %s", lat, lng, exc)
            raise FootprintUnavailable(str(exc)) from exc
        cache.put(lat, lng, neighbor_radius, (elements, source))
        was_cached = False

    query_point = (lng, lat)
    parsed = [c for c in (to_candidate(e, query_point) for e in elements) if c is not None]
    ...
    selected, confidence, reason = resolve(candidates, query_point)
