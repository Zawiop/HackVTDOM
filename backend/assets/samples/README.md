...
### Three of these were refit (2026-09-20)

Three meshes were unusable as generated. `holden_flooded.glb` and `derring_flooded.glb` baked
**blank texture atlases** — flat white panels where the others carry windows and cornices — and
`hancock_flooded.glb` uses only 38% of its atlas with UVs that sample the black padding. All
three rendered as featureless blocks on the map: two grey slabs and a dark cone.

Each was replaced by a *different* good mesh, uniformly scaled and normalized onto its own
hall's footprint — the same technique that produced most of this set in the first place (one
generated building, refit to each hall's real OSM outline). Different sources on purpose, so
the skyline stays varied instead of becoming three copies of one building:

| hall | refit from |
| --- | --- |
| `holden_flooded.glb` | `norris_flooded.glb` |
| `derring_flooded.glb` | `hitt_flooded.glb` |
| `hancock_flooded.glb` | `burruss_flooded.glb` |

Each `.mesh-response.json` carries a `note` saying so, and those three rows in
