...
from app.services import footprint
...
def test_way_element_parses_to_candidate():
    ...
    candidate = footprint.to_candidate(_way(1, ring), (LNG, LAT))
...
def test_relation_outer_members_stitch_into_one_ring():
    ...
    candidate = footprint.to_candidate(relation, (LNG, LAT))
    assert candidate is not None
...
def test_single_containing_footprint_is_high_confidence():
    candidate = footprint.to_candidate(_way(1, _square(LNG, LAT, 40.0)), (LNG, LAT))
    selected, confidence, _ = footprint.resolve([candidate], (LNG, LAT))
...
def test_no_candidates_is_low_confidence():
    selected, confidence, reason = footprint.resolve([], (LNG, LAT))
...
def test_equally_close_candidates_are_never_auto_picked():
    ...
    candidates = [
        footprint.to_candidate(left, (LNG, LAT)),
        footprint.to_candidate(right, (LNG, LAT)),
    ]
    candidates.sort(key=lambda c: c.distanceMeters)
    selected, confidence, reason = footprint.resolve(candidates, (LNG, LAT))
...
def test_clearly_nearest_candidate_is_selected():
    ...
    candidates = [
        footprint.to_candidate(near, (LNG, LAT)),
        footprint.to_candidate(far, (LNG, LAT)),
    ]
    candidates.sort(key=lambda c: c.distanceMeters)
    selected, confidence, _ = footprint.resolve(candidates, (LNG, LAT))
...
def test_overlapping_footprints_are_ambiguous():
    ...
    candidates = [
        footprint.to_candidate(a, (LNG, LAT)),
        footprint.to_candidate(b, (LNG, LAT)),
    ]
    selected, confidence, reason = footprint.resolve(candidates, (LNG, LAT))
...
def test_malformed_elements_are_dropped_not_fatal():
    assert footprint.to_candidate({"type": "way", "id": 1}, (LNG, LAT)) is None
    assert footprint.to_candidate(
